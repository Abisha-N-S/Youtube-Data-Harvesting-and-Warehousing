...
def Channel_table():
    ...
    #Inserting the datas into Channel table
    client = pymongo.MongoClient("mongodb://127.0.0.1:27017/")
    db=client["Youtube"]
    col=db["Channel_detail"]
    mongo_data =col.find()
...
def Video_table():
    ...
    #Inserting datas into Video table
    client = pymongo.MongoClient("mongodb://127.0.0.1:27017/")
    db=client["Youtube"]
    col=db["Channel_detail"]
    mongo_data =col.find()
...
def Comment_table():
    ...
    #Inserting datas into Comment table
    client = pymongo.MongoClient("mongodb://127.0.0.1:27017/")
    db=client["Youtube"]
    col=db["Channel_detail"]
    mongo_data =col.find()
...
#Streamlit page side bar creation
with st.sidebar:
...
#Fetch channel, video  and comment details for specified channel_id and insert into MongoDB
def Channel_detail(channel_id):
...
def fetch_all_channel_ids():
    ...
    return channel_ids
#Collect and store data into MongoDB
...
#Inserting all details from MongoDB to Postgres
def chan():
    mydb = psycopg2.connect(
        host="localhost",
        user="postgres",
